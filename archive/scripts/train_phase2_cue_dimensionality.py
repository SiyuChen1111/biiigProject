...
LOW_DIMENSIONALITY = 1
HIGH_DIMENSIONALITY = 4
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train phase-2 EEGNet on cue_dimensionality extremes.")
    parser.add_argument("--dataset-dir", type=Path, default=DATASET_DIR)
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/phase2_cue_dimensionality"))
...
def _load_subject_dataset(subject_id: str, dataset_dir: Path) -> SubjectDataset:
    ...
    keep_mask = trial_table["cue_dimensionality"].isin([LOW_DIMENSIONALITY, HIGH_DIMENSIONALITY]).to_numpy()
    filtered_trial_table = trial_table.loc[keep_mask].reset_index(drop=True)
    epoched = extract_response_locked_epochs(
        subject_id=subject_id,
        trial_table=filtered_trial_table,
...
def _label_from_cue_dimensionality(trial_table: pd.DataFrame) -> np.ndarray:
    values = trial_table["cue_dimensionality"].to_numpy()
    labels = np.full(values.shape[0], fill_value=-1, dtype=np.int64)
    labels[values == LOW_DIMENSIONALITY] = 0
    labels[values == HIGH_DIMENSIONALITY] = 1
    if np.any(labels < 0):
        raise ValueError("Found unexpected cue_dimensionality values after filtering.")
    return labels


def _make_labeled_block(block: SubjectDataset) -> LabeledSubjectBlock:
    labels = _label_from_cue_dimensionality(block.trial_table)
    return LabeledSubjectBlock(
        epochs=block.epochs,
        labels=labels,
        trial_table=block.trial_table,
        times=block.times,
...
def main() -> None:
    ...
    for fold_idx, test_subjects in enumerate(fold_assignments, start=1):
        ...
        train_block = _make_labeled_block(_concatenate_subject_blocks([subject_blocks[s] for s in train_subjects]))
        val_block = _make_labeled_block(_concatenate_subject_blocks([subject_blocks[s] for s in val_subjects]))
        test_block = _make_labeled_block(_concatenate_subject_blocks([subject_blocks[s] for s in test_subjects]))

        x_train = _epochs_to_model_input(train_block.epochs)
        x_val = _epochs_to_model_input(val_block.epochs)
        x_test = _epochs_to_model_input(test_block.epochs)
        y_train = train_block.labels
        y_val = val_block.labels
        ...
        combined_subject_metrics.update(
            _subject_metrics(
                trial_table=test_block.trial_table,
                y_true=y_test_true,
                y_pred=y_test_pred,
                y_prob=y_test_prob,
            )
        ...
        plot_roi_erp_comparison(
            epochs=test_block.epochs,
            labels=y_test_pred,
            times=test_block.times,
            channel_names=test_block.channel_names,
            output_dir=fold_dir,
