...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train phase-1 EEGNet on Kosciessa task-dynamic.")
    parser.add_argument("--dataset-dir", type=Path, default=DATASET_DIR)
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/phase1_eegnet"))
...
def _evaluate_loader(model: EEGNet, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    model.eval()
    ...
    mean_loss = loss_total / max(1, len(loader))
...
def main() -> None:
    ...
    for fold_idx, test_subjects in enumerate(fold_assignments, start=1):
        ...
        label_transform = fit_cpp_label_transform(train_block.features)
        train_ready = _apply_label_transform(train_block, label_transform)
        val_ready = _apply_label_transform(val_block, label_transform)
        test_ready = _apply_label_transform(test_block, label_transform)
        ...
        for _ in range(args.epochs):
            ...
            y_val_true, y_val_pred, y_val_prob, _ = _evaluate_loader(model=model, loader=val_loader, device=device)
        ...
        y_test_true, y_test_pred, y_test_prob, _ = _evaluate_loader(model=model, loader=test_loader, device=device)
        ...
        per_subject = _subject_metrics(
            trial_table=test_ready.trial_table,
            y_true=y_test_true,
            y_pred=y_test_pred,
            y_prob=y_test_prob,
        )
        combined_subject_metrics.update(per_subject)
